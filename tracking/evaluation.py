...
def evaluate(
    ...
    ego_distance_threshold: float = 50,
) -> Dict:
    """
    ...
    """
    labels = filter_by_ego_xy_distance(
        labels, distance_threshold=ego_distance_threshold
    )
    annotate_frame_metadata(
        ungroup_frames(track_predictions), ungroup_frames(labels), ["ego_translation"]
    )
    track_predictions = filter_by_ego_xy_distance(
        track_predictions, distance_threshold=ego_distance_threshold
    )
    labels_id_ts = set(
